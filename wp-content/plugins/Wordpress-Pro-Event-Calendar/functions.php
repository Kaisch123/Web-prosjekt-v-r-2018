...
function dpProEventCalendar_bookEvent() {
	...
	if($price == "" && $ticket == '') {
		...
		wp_mail( $userdata->user_email, get_bloginfo('name'), apply_filters('pec_booking_email', $dpProEventCalendar_class->calendar_obj->booking_email_template_user, $id_event, $userdata->display_name, $userdata->user_email, $event_date, $comment, $quantity, $phone), $headers );
		
		// Email to Author
		$event_author_id = get_post_field( 'post_author', $id_event );
		
		wp_mail( get_the_author_meta( 'user_email', $event_author_id ), get_bloginfo('name'), apply_filters('pec_booking_email', $dpProEventCalendar_class->calendar_obj->booking_email_template_admin, $id_event, $userdata->display_name, $userdata->user_email, $event_date, $comment, $quantity, $phone), $headers );
	}
...
add_filter('pec_booking_email', 'dpProEventCalendar_bookingEmail', 10, 8);
...
if(!$dpProEventCalendar['disable_rewrite_rules']) {
...
function dpProEventCalendar_pro_event_calendar_taxonomies() 
{
  ...
  $args = array(
    ...
    'query_var'           => true,
    'show_in_rest'       => true,
    'rewrite'             => array( 'slug' => ( $dpProEventCalendar['categories_slug'] != "" ? $dpProEventCalendar['categories_slug'] : 'pec_events_category') )
  );
	...
	$querystr = "
		...
		if(!empty($bookings_obj)) {
			...
			foreach($bookings_obj as $booking) {
				...
				// Email to User
				wp_mail( $userdata->user_email, get_bloginfo('name'), apply_filters('pec_booking_email', $booking_email_template_reminder_user, $booking->id_event, $userdata->display_name, $userdata->user_email, $booking_search_date, $booking->comment, $booking->quantity, $booking->phone), $headers );
			}
...
function dpProEventCalendar_importFB($calendar_id, $event_url, $category_ics = '', $event_option = 2, $offset = '') {
	...
	if($event_option == 2) {
		...
		if(strlen($last_chars) > 10 && is_numeric($last_chars)) {

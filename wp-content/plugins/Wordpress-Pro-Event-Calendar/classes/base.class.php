...
class DpProEventCalendar {
		...
		switch($get) {
			...
			case 'date':
				...
				if(!$event_data->tbc) {
					...
					$time = dpProEventCalendar_date_i18n(get_option('time_format'), strtotime($event_data->date));
					...
					if($end_time != "") {
						...
						$end_time_tmp = dpProEventCalendar_date_i18n(get_option('time_format'), strtotime("2000-01-01 ".$end_time.":00"));
	...
	function getBookingForm($post_id, $date = '') {
		...
		if(is_array($dpProEventCalendar['booking_custom_fields_counter']) && !$hide_buttons) {
			...
			foreach($dpProEventCalendar['booking_custom_fields_counter'] as $key) {
				...
				if(!empty($cal_booking_custom_fields) && $cal_booking_custom_fields != "" && !in_array($dpProEventCalendar['booking_custom_fields']['id'][$counter], $cal_booking_custom_fields_arr)) {
					$counter++;
					continue;
				}
			...
			if(!is_array($event_dates) || count($event_dates) == 0) {
				...
				foreach($event_dates as $upcoming_date) {
					...
					if($upcoming_date->end_time_hh != "" && $upcoming_date->end_time_mm != "") {
						...
						$end_time = dpProEventCalendar_date_i18n(get_option('time_format'), strtotime("2000-01-01 ".$end_time.":00"));

						if($end_time == dpProEventCalendar_date_i18n(get_option('time_format'), strtotime($upcoming_date->date))) {
							$end_time = "";	
						}
					}
					...
					$option_text = dpProEventCalendar_date_i18n(
						...
						get_option('date_format') . ($this->calendar_obj->show_time && !$upcoming_date->hide_time ? ' ' . get_option('time_format') : ''), 
		...
		//die();
			...
			for($i = 1; $i <= $this->datesObj->daysInCurrentMonth; $i++) 
			{
				...
				if(($this->calendar_obj->date_range_start != '0000-00-00' && $this->calendar_obj->date_range_start != NULL && (strtotime($curDate) < strtotime($this->calendar_obj->date_range_start)) || ( $this->calendar_obj->date_range_end != '0000-00-00' && $this->calendar_obj->date_range_end != NULL && strtotime($curDate) > strtotime($this->calendar_obj->date_range_end))) && !$this->is_admin) {
					...
					$html .= '
						<div class="dp_pec_date dp_pec_date_'.strtolower(date('l', strtotime($curDate))).' '.($countEvents > 0 && !$booked_date && $booking_remain ? 'pec_has_events' : '').' '.($general_count % 7 == 0 ? 'first-child' : '').' '.($special_date != "" ? 'dp_pec_special_date' : '').'" data-dppec-date="'.$curDate.'" '.$special_date.'>
							<div class="dp_date_head"><span>'.$i.'</span></div>
							';
					...
					if($this->calendar_obj->show_titles_monthly && empty($this->event_id) && !$compact) {
						...
						foreach($eventsCurrDate as $event) {
							...
							$time = dpProEventCalendar_date_i18n(get_option('time_format'), strtotime($event->date));
				...
				<div class="dp_pec_row">
					...
					for($x = 1; $x <= 7; $x++) {
						...
						if(isset($weekly_events_view[$x][$i])) {
							...
							$html .= '
							<div class="dp_pec_date dp_pec_date_'.strtolower(date('l', strtotime($curDate))).' '.$disabled.'" '.($x == 1 ? 'style="margin-left: 0;"' : '').'></div>';
	...
	function eventsListLayout($date, $return_btn = true) {
		...
		<div class="dp_pec_columns_1 dp_pec_isotope dp_pec_date_event_wrap dp_pec_date_block_wrap">
			...
			';
			if($return_btn) {
				$html .= '<a href="#" class="dp_pec_date_event_back dp_pec_btnright"><i class="fa fa-angle-double-left"></i> <span>'.$this->translation['TXT_BACK'].'</span></a>';
			}
		$html .= '
		</div>
		<div class="dp_pec_clear"></div>';	
	...
	function getSearchResults($key, $type = '') {
		...
		if($type == '') {
			...
			<div class="dp_pec_date_block_wrap dp_pec_date_event_search dp_pec_isotope">
				...
				<a href="#" class="dp_pec_date_event_back dp_pec_btnright"><i class="fa fa-angle-double-left"></i> <span>'.$this->translation['TXT_BACK'].'</span></a>
			</div>
			<div class="dp_pec_clear"></div>';
		}
		...
		if(count($events_obj) == 0) {
						...
						switch($event->recurring_frecuency) {
							...
							case 2:
								...
								if(is_array($pec_weekly_day)) {
									...
									$last_day = date("Y-m-d H:i:s", mktime(date("H", strtotime($event_date)), date("i", strtotime($event_date)), 0, date("m", strtotime($event_date)), date("d", strtotime($event_date)), date("y", strtotime($event_date))) - 86400);
			...
			if(empty($order_events)) {
				...
				foreach($order_events as $event) {
					...
					if((is_numeric($this->columns) && $this->columns > 1) || $event->tbc ) {
						if($event->tbc) {
							$html .= '<span class="dp_pec_date_time"><i class="fa fa-calendar-o"></i>'.$this->translation['TXT_TO_BE_CONFIRMED'].'</span>';
						} else {
							$html .= '<span class="dp_pec_date_time"><i class="fa fa-calendar-o"></i>'.$start_date.$end_date.'</span>';
	...
	function output( $print = false ) 
	{
		...
		if($this->type == 'calendar') {
			...
			$event_list = $this->upcomingCalendarLayout( true, ($this->limit + 1), '', null, null, true, false, false );
					...
					foreach ($event_list as $event) {
						...
						$html .= '<div class="dp_pec_date_event" data-event-number="1">
									<div class="dp_pec_date_left">
										<div class="dp_pec_date_left_number">'.$day_number.'</div>
										<div class="dp_pec_date_left_month">'.$month.'</div>
									</div>
			...
			$html .= '
				...
				if(is_array($event_list)) {
					foreach ($event_list as $event) {
						...
						$html .= '<div class="dp_pec_countdown_event" data-countdown-tzo="'.$tzo.'" data-current-year="'.current_time("Y").'" data-current-month="'.current_time("m").'" data-current-day="'.current_time("d").'" data-current-hour="'.current_time("H").'" data-current-minute="'.current_time("i").'" data-countdown-year="'.date("Y",$time).'" data-countdown-month="'.date("m",$time).'" data-countdown-day="'.date("d",$time).'" data-countdown-hour="'.date("H",$time).'" data-countdown-minute="'.date("i",$time).'">';
			...
			if(is_array($event_list)) {
				...
				foreach ($event_list as $obj) {
					...
					if(!is_object($unique_events[$obj->id])) {
						...
						$time = dpProEventCalendar_date_i18n(get_option('time_format'), strtotime($obj->date));
			...
			if(!is_array($bookings_list) || empty($bookings_list)) {
				...
				foreach($bookings_list as $key) {
					...
					switch($key->status){
						...
						$html .= '<span class="dp_pec_date_time"><i class="fa fa-clock-o"></i>'.dpProEventCalendar_date_i18n(get_option('date_format'), strtotime($key->event_date)). ' ' .dpProEventCalendar_date_i18n(get_option('time_format'), strtotime(get_post_meta($key->id_event, 'pec_date', true))).'</span>

...
class WDDApi {
  ...
  private function get_installed_themes_slugs() {
    ...
    foreach ($themes as $key => $theme) {
      if ((strpos($theme->Author, "web-dorado.com") !== false || strpos($theme->Author, "WebDorado") !== false) && $this->check_existing_file("themes/" . $key)) {
  ...
  private function get_installed_plugins_slugs() {
    ...
    foreach ($all_plugins as $key => $all_plugin) {
      ...
      if (($valid_webdorado_URI !== false ) && ($this->check_existing_file("plugins/" . $slug) || $key == WDD_SLUG)) {

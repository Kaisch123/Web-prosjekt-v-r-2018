...
jQuery( document ).ready(function() {
    ...
    EDITOR = jQuery('.hidden-editor-container').contents();
	...
	jQuery(".wd-pois-tabs li a").click(function(){
		...
		jQuery(".wd-pois-tabs li a").removeClass("wd-pois-active-wd-gm-circles-tab");
		...
		jQuery(".wd-pois-tabs li a").removeClass("wd-pois-active-wd-gm-rectangles-tab");        
		jQuery(jQuery(this).attr("href")).show();
   ...
   // db-click zoom
	...
	jQuery("#fusion_table_id, #fusion_table_where_filed, #fusion_table_where_operator, #fusion_table_where_value ").change(function(){
		fusionTableId = jQuery("#fusion_table_id").val();
        fusionTableWhereField = jQuery("#fusion_table_where_filed").val();
        fusionTableWhereOperator = jQuery("#fusion_table_where_operator :selected").val();
        fusionTableWhereValue = jQuery("#fusion_table_where_value").val();
		gmwdSetFusionTableId();
	});
    jQuery("#fusion_table_where_operator").change(function(){
        if(jQuery(this).val() == "IN"){
            jQuery(".gmwd_eg_fusion_table_where_value").html("e.g. ('RU', 'AM', 'BE')");            
        }
        else{
            jQuery(".gmwd_eg_fusion_table_where_value").html("e.g. BE");  
        }
    ...
    jQuery("[name=infowindow_type]").change(function(){
        if(jQuery(this).val() == "0"){
            jQuery(".advanced_info_window_position_tr").hide();
        }
        else{
            jQuery(".advanced_info_window_position_tr").show();
        }
    });	
    
    // advanced table columns
    
	jQuery("#marker_listing_type").change(function(){
        if(jQuery(this).val() == 2){
            jQuery(".tr_advanced_columns").show();
        }
        else{
            jQuery(".tr_advanced_columns").hide();
        }
    }); 
    
	jQuery(".advanced_table_columns").change(function(){
        var selectedColumns = [];
        jQuery(".advanced_table_columns:checked").each(function(){
            selectedColumns.push(jQuery(this).val());
        });
        jQuery("[name=advanced_table_columns]").val(selectedColumns.join());
    }); 
...
function gmwdUpdatePreview(){
    ...
    */
    ...
    previewIframeURL = previewIframeURL + "&theme_id=" + jQuery("[name=theme_id]:checked").val();
...
function gmwdOpenPoiForm(obj, data){
    ...
    tinymce.execCommand( 'mceRemoveEditor', false, 'mdescription' );
    jQuery.ajax({
        type: "POST",
        url: url,
        ...
        success: function (result) {  
            ...
            switch(poi){
                case "markers" :
                    jQuery('.gmwd_editor').html(EDITOR);                    
                    tinymce.execCommand( 'mceAddEditor', false, 'mdescription' );
                ...
                case "circles" :
                    ...
                    initAutocomplete(true, gmwdAddCircle,"circle_address","lat", "lng");
                    gmwdCircleMapEvents();               
                    break;  
                case "rectangles" :
					if(url.indexOf("&hiddenName=") > -1){
	...
	switch(type){
		...
		case "circles" :
			if(data != false)
				...
				delete mapCircles[key];	
				
			for(var i=0; i<allCircleMarkers.length; i++){
				allCircleMarkers[i].setMap(null);
			}
		...
		case "rectangles" :
			if(data != false)
				mapRectangles[key] = data;
			else
				delete mapRectangles[key];	
				
			for(var i=0; i<allRectangleMarkers.length; i++){
				allRectangleMarkers[i].setMap(null);
			}
...
function removePoi(obj, single){	
	...
	switch(type){
		case "markers" :
			...
			break;
		case "circles" :
			mapCircles[key] === undefined;

			break;	
		case "rectangles" :
			mapRectangles[key] === undefined;
			break;	
...
function fillInputPois(){
    ...
    jQuery(".gmwd_markers .wd-pois-row:not(.wd-template) ").each(function(index){        
	...
	jQuery(".gmwd_circles .wd-pois-row:not(.wd-template)").each(function(){
		var circle = jQuery(this).find("[name^=circles_]").val();
		circles.push(JSON.parse(circle));
	});
	...
	jQuery(".gmwd_rectangles .wd-pois-row:not(.wd-template)").each(function(){
		var rectangle = jQuery(this).find("[name^=rectangles_]").val();  
		rectangles.push(JSON.parse(rectangle));
	});	
	...
	jQuery("[name=circles]").val(JSON.stringify(circles));

	jQuery("[name=polygons]").val(JSON.stringify(polygons));
	jQuery("[name=polylines]").val(JSON.stringify(polylines));
	jQuery("[name=rectangles]").val(JSON.stringify(rectangles));

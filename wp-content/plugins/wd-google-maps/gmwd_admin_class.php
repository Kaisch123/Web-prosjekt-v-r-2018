...
class GMWDAdmin
{
    ...
    public function __construct()
    {
        ...
        if (!isset($_REQUEST['ajax'])) {
            require_once(GMWD_DIR . '/update/updates.php');
            $gmwd_update = new GMWD_Update();
            add_action('admin_menu', array($gmwd_update, 'check_for_update'), 25);
        }
    ...
    {
        delete_transient('gmwd_remote_data');
        ...
        if (get_option("gmwd_pro")) {
            update_option("gmwd_pro", "yes");
        } else {
            add_option("gmwd_pro", "yes", '', 'no');
        }
        ...
        if (!file_exists(GMWD_UPLOAD_DIR)) {
            wp_mkdir_p(GMWD_UPLOAD_DIR);
            $marker_types = array("clothtexture", "coloring", "retro", "custom", "modern", "papertexture", "standart", "woodtexture");
            foreach ($marker_types as $marker_type) {
                if (!file_exists(GMWD_UPLOAD_DIR . '/markers/' . $marker_type)) {
                    wp_mkdir_p(GMWD_UPLOAD_DIR . '/markers/' . $marker_type);
                }
            }
        }
    ...
    public function gmwd_styles()
    {
        ...
        if ($page == "themes_gmwd") {
            wp_enqueue_style('gmwd_bootstrap-css', GMWD_URL . '/css/bootstrap.css', array(), self::$version);
            wp_enqueue_style('gmwd_frontend-css', GMWD_URL . '/css/frontend_main.css', array(), self::$version);
            wp_enqueue_style('gmwd_owl-css', GMWD_URL . '/css/owl.carousel.css', array(), self::$version);
        }
    ...
    public function gmwd_scripts()
    {
        ...
        wp_enqueue_script('gmwd_util-js', GMWD_URL . '/js/util.js');
        ...
        if (gmwd_get_option("map_api_key")) {
            ...
            //AIzaSyDN6xxpICiP9LcwfqqN2yytG8tMJyCp56A
        ...
        $page = GMWDHelper::get('page') ? GMWDHelper::get('page') : "maps_gmwd";
        if ($page == "markers_gmwd" || $page == "circles_gmwd" || ($page == "maps_gmwd" && GMWDHelper::get('task') == "edit") || $page == "polygons_gmwd" || $page == "polylines_gmwd" || $page == "rectangles_gmwd" || $page == "markercategories_gmwd" || $page == "themes_gmwd" || $page == "options_gmwd") {
            ...
            wp_enqueue_script($page . '-js', GMWD_URL . '/js/' . $page . '.js', array(), self::$version);
        }

        if ($page == "maps_gmwd" && GMWDHelper::get('task') == "edit") {
            wp_enqueue_script('gmwd_init_map_admin-js', GMWD_URL . '/js/init_map_admin.js', array(), self::$version);

        }
        if ($page == "themes_gmwd") {
            wp_enqueue_script('gmwd_angular-js', 'https://ajax.googleapis.com/ajax/libs/angularjs/1.5.0/angular.min.js');
            wp_enqueue_script('gmwd_owl-js', GMWD_URL . '/js/owl.carousel.js', array(), self::$version, true);
        }
        /*        if($page == "uninstall_gmwd") {		   

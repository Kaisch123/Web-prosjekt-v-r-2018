...
class GMWDModelMaps_gmwd extends GMWDModel {
	...
	public function get_row($id){
		...
		if($id){
			...
			$limit_circles = isset($_POST["limit_circles"]) ? (int)$_POST["limit_circles"] : 20;	
			$limit_rectangles = isset($_POST["limit_rectangles"]) ? (int)$_POST["limit_rectangles"] : 20;	
			...
			$filter_by_circles = isset($_POST["filter_by_circles"]) ? esc_html(stripslashes($_POST["filter_by_circles"])) : "";
			$filter_by_rectangles = isset($_POST["filter_by_rectangles"]) ? esc_html(stripslashes($_POST["filter_by_rectangles"])) : "";
			...
			$where_circles = "";
			if($filter_by_circles){
				$where_circles = " AND title LIKE '%".$filter_by_circles."%'";
			}
			$where_rectangles = "";
			if($filter_by_rectangles){
				$where_rectangles = " AND title LIKE '%".$filter_by_rectangles."%'";
			}            
			$where_polygons = "";
			...
			foreach($markers as $marker){
                $marker->description = htmlspecialchars_decode($marker->description) ? preg_split('/\r\n|\r|\n/', htmlspecialchars_decode($marker->description)) : array(); 
			...
			// circles
			$circles = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "gmwd_circles WHERE map_id= '".$id."' ".$where_circles."  ORDER BY id DESC LIMIT 0,".$limit_circles);            
			$all_circles = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "gmwd_circles WHERE map_id= '".$id."' ".$where_circles."");	
            $row->circles_count = count($all_circles);            
			$row_circles = array();
			foreach($circles as $circle){
                $circle->title = str_replace(array('"', '&quot;'),"@@@",$circle->title);
				$row_circles[$circle->id] = $circle;			
			}
			$row->circles = $row_circles;
            
			$row_all_circles = array();
			foreach($all_circles as $circle){
				$row_all_circles[$circle->id] = $circle;			
			}
			$row->all_circles = $row_all_circles;            
            
            //rectangles
            $rectangles = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "gmwd_rectangles WHERE map_id= '".$id."' ".$where_rectangles."  ORDER BY id DESC LIMIT 0,".$limit_rectangles);
           
			$all_rectangles = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "gmwd_rectangles WHERE map_id= '".$id."' ".$where_rectangles."");
            $row->rectangles_count= count($all_rectangles);		
			$row_rectangles = array();
			foreach($rectangles as $rectangle){
                $rectangle->title = str_replace(array('"', '&quot;'),"@@@",$rectangle->title);
				$row_rectangles[$rectangle->id] = $rectangle;			
			}
			$row->rectangles = $row_rectangles;
            
			$row_all_rectangles = array();
			foreach($all_rectangles as $rectangle){
				$row_all_rectangles[$rectangle->id] = $rectangle;			
			}
			$row->all_rectangles = $row_all_rectangles;            
			...
			$row->marker_listing_order = "title";
			$row->marker_listing_order_dir = "asc";

			$row->directions_header_title = "Get Directions";
			$row->store_locator_header_title = "Store Locator";
			$row->listing_header_title = "Markers";
			$row->advanced_table_columns = array("icon", "title", "category", "address", "desc");
			...
			$row->markers_count = 0;
			$row->circles = array();
			$row->all_circles = array();
			$row->circles_count = 0;
            $row->rectangles = array();
            $row->all_rectangles = array();
			$row->rectangles_count = 0;
			$row->polygons = array();
    ...
    public function get_theme($theme_id){
		global $wpdb;
		// get rows
		$query = "SELECT *  FROM " . $wpdb->prefix . "gmwd_themes WHERE id='".$theme_id."'";				
		$theme = $wpdb->get_row($query);
		return $theme;	    
	...
	public function get_lists(){
        ...
        $fusion_table_where_operators_list = array("=" => "=", ">" => ">", "<" => "<", ">=" => ">=" , "<=" => "<=", "IN" => "IN", "!=" => "!=");
 
        $marker_category_types_list = array( __("Dropdown List","gmwd"), __("Tabs","gmwd"));
        $inside_map_marker_category_types_list = array( __("List","gmwd"), __("Tabs","gmwd"));

		$marker_listing_order_list = array( "id" => __("ID","gmwd"), "title" => __("Title","gmwd"), "address" => __("Address","gmwd"), "description" => __("Description","gmwd") );
		...
		$lists["fusion_table_where_operators"] = $fusion_table_where_operators_list;
		$lists["marker_category_types"] = $marker_category_types_list;
		$lists["inside_map_marker_category_types"] = $inside_map_marker_category_types_list;
		$lists["marker_listing_order"] = $marker_listing_order_list;
	...
	public function get_query_urls($map_id){
		...
		//add circles url
		$query_url_circle = add_query_arg(array('action' => 'add_circle', 'page' => 'circles_gmwd', 'task' => 'edit','map_id' => $map_id, 'nonce_gmwd' => wp_create_nonce('nonce_gmwd')), $query_url);
        
        //add circles url
		$query_url_rectangle = add_query_arg(array('action' => 'add_rectangle', 'page' => 'rectangles_gmwd', 'task' => 'edit','map_id' => $map_id, 'nonce_gmwd' => wp_create_nonce('nonce_gmwd')), $query_url);
        
        // export url
		$query_url_export = add_query_arg(array('action' => 'export', 'page' => 'maps_gmwd', 'task' => 'export','map_id' => $map_id,'ajax' => 1, 'nonce_gmwd' => wp_create_nonce('nonce_gmwd')),$query_url);
        
        // import url
		$query_url_import = add_query_arg(array('action' => 'import', 'page' => 'maps_gmwd', 'task' => 'import','map_id' => $map_id, 'ajax' => 1, 'nonce_gmwd' => wp_create_nonce('nonce_gmwd')),$query_url);

		$urls["marker"] = $query_url_marker;
		$urls["polygon"] = $query_url_polygon;
		$urls["polyline"] = $query_url_polyline;
		$urls["circle"] = $query_url_circle;
		$urls["rectangle"] = $query_url_rectangle;
		$urls["export"] = $query_url_export;
		$urls["import"] = $query_url_import;

...
class GMWDViewMarkers_gmwd extends GMWDView{
	...
	?> 
      <div class="gmwd_editor_wrapper">
            <div class="wd-text-right" style="padding:10px;">        
                <button class="wd-btn wd-btn-primary insert_editor_text" onclick="return false;"><?php _e("Insert","gmwd");?></button>
                <button class="wd-btn wd-btn-secondary cancel_editor" onclick="return false;"><?php _e("Cancel","gmwd");?></button>            
            </div>
            <div class="gmwd_editor">
					...
					<div class="wd-left">
						<table class="pois_table">
							<tr>
								...
								<td>
                                    <input type="text" name="link_url" id="link_url" value="<?php echo $row->link_url;?>" style="width:auto;" class="wd-form-field">
                                </td>                                
							</tr>							
							<tr>
								...
								<td>
                                    <button class="wd-btn wd-btn-primary" onclick="gmwdOpenMediaUploader(event,'pic_url',gmwdCreateInfoWindow);return false;"><?php _e("Upload Image","gmwd"); ?></button>
                                    <input type="hidden" name="pic_url" id="pic_url" value="<?php echo $row->pic_url; ?>" class="wd-form-field">                                   
                                    <div class="pic_url_view upload_view">
                                       <?php if($row->pic_url){
                                            echo '<img src="'.$row->pic_url.'" height="18">';
                                            echo '<span class="pic_url_delete" onclick="jQuery(\'#pic_url\').val(\'\');jQuery(\'.pic_url_view\').html(\'\');gmwdCreateInfoWindow();">x</span>';
                                        }
                                        ?>               
                                    </div> 
                                    <small><?php _e("Basic Table Marker List Type Image as Well.","gmwd");?></small>                                    
                               </td>							
                            </tr>
							<tr>
								...
								<td>
									<?php
										...
										foreach($row->categories as $key => $value){
											...
											?>
												<div>
													<input type="checkbox" id="cat<?php echo $key;?>" class="categories" value="<?php echo $key; ?>" <?php echo $checked; ?> >			
													<label for="cat<?php echo $key;?>"><?php echo $value; ?></label>
									...
									?>
									<input type="hidden" name="category" id="category" class="wd-form-field" value="<?php echo $row->category;?>">
								</td>							
                            </tr>					
							<tr>
							...
							<tr>
								...
								<td>
                                    <textarea name="description" id="description" class="wd-form-field"><?php echo $row->description;?></textarea>
                                   
                                </td>
                                <td colspan="2">
                                    <button class="wd-btn wd-btn-primary open_editor" onclick="return false;"><?php _e("Open Editor","gmwd");?></button>
                                </td>
                            ...
                            <tr>
								...
								<td colspan="2">
                                  <input type="radio" class="inputbox wd-form-field" id="choose_marker_icon1" name="choose_marker_icon" <?php echo ($row->choose_marker_icon == 1  ? 'checked="checked"' : ''); ?> value="1" >
								  <label for="choose_marker_icon1"><?php _e("Choose from Icons","gmwd"); ?></label>
								  <input type="radio" class="inputbox wd-form-field" id="choose_marker_icon0" name="choose_marker_icon" <?php echo (($row->choose_marker_icon == 0 ) ? 'checked="checked"' : ''); ?> value="0"  >
                            ...
                            <tr>  
                                ...
                                <td>
                                    <div class="from_media_uploader" <?php echo (($row->choose_marker_icon == 0 ) ? '' : 'style="display:none;"'); ?>>
                                        <button class="wd-btn wd-btn-primary" onclick="gmwdOpenMediaUploader(event,'custom_marker_url',gmwdSetMarkerIcon);return false;"><?php _e("Upload Marker Image","gmwd"); ?></button>
                                    ...
                                    <div class="custom_marker_url_view upload_view">
                                       <?php if($row->custom_marker_url){
                                            echo '<img src="'.$row->custom_marker_url.'" height="25">';
                                            echo '<span class="custom_marker_url_delete" onclick="jQuery(\'#custom_marker_url\').val(\'\');jQuery(\'.custom_marker_url_view\').html(\'\');gmwdSetMarkerIcon();">x</span>';
                                        }
                                        ?>               
                                    </div> 
                                </td>                               
							...
							<tr>
								...
								<td>
									<select name="marker_size" id="marker_size" class="wd-form-field">
										<?php 
			...
			<div class="wd-row wd-table" style="width:100%">
				...
				<div class="wd-marker-tabs-container wd-cell wd-cell-valign-top">
                                ...
                                <?php
                                    ...
                                    ?>
                                        <div class="icon-block wd-left" onclick="gmwdChooseIcon(this);" data-icon-type="<?php echo str_replace("_", "", $marker_key);?>" data-icon-number="<?php echo $i;?>">
                                            <img src="<?php echo GMWD_UPLOAD_URL . '/markers/'.str_replace("_", "", $marker_key).'/'.str_replace("_", "", $marker_key).'_'.$i.'_64.png';?>" width="60" style="vertical-align:middle;">
                                        </div>
                                    <?php
                                    }
                                }
                    ...
                    <div id="marker-added" class="wd-marker-container">
                        <div class="wd-clear">
                            <?php   
                                $added_markers = scandir(GMWD_UPLOAD_DIR . "/markers/custom");
                                ...
                                if(count($added_markers) > 2){
                                    ...
                                    ?>
                                        <div class="icon-block wd-left" onclick="gmwdChooseIcon(this);" data-icon-type="custom_added" data-icon-number="0">
                                            <img src="<?php echo GMWD_UPLOAD_URL . '/markers/custom/'.$added_markers[$i];?>" width="60" style="vertical-align:middle;">
                                        </div>
                                    <?php
                                    }
                                }

<?php
/**
 * Fotografie Theme Options Sanitization Functions
 *
 * @package Fotografie
 */
 ...
 */
function fotografie_sanitize_post( $input ) {
 ...
 */
function fotografie_sanitize_image( $image, $setting ) {
    /*
 ...
 */
function fotografie_sanitize_category_list( $input ) {
    if ( '' != $input ) {
        if ( in_array( 0, $input ) ) {
            return '0';
        }
        ...
        if ( is_array( $input ) && is_array( $category_list ) ) {
            // Intersect array with all category list array. if number is more, then there are more items, so sanitization fails.
 ...
 *                    the setting default.
 */
function fotografie_sanitize_number_range( $number, $setting ) {
